...
S3_PREFIX = "data_raw/visits/"
MONGO_DB = "test"
MONGO_COLLECTION = "visits"
AIRFLOW_VAR = "mongo_visits_processed_count"
# ==================
...
def mongo_visits_batch_upload():
    ...
    file_name = f"visits_batch_{batch_number}.json"
    ...
    Variable.set("visits_data_ready", "true")

    print(f"Uploaded visits batch ending at record {batch_number}")


with DAG(
    dag_id="mongo_visits_to_s3",
    start_date=datetime(2024, 1, 1),
    schedule_interval="*/5 * * * *",
    max_active_runs=1,
    catchup=False,
    tags=["mongo", "visits", "s3"]
) as dag:

    upload_task = PythonOperator(
        task_id="mongo_visits_batch_upload",
        python_callable=mongo_visits_batch_upload
    )
